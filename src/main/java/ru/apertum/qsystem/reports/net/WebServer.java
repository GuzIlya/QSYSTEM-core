...
public class WebServer {
    ...
    public synchronized void startWebServer(int port) {
        ...
        log.info("Отчетный сервер захватывает порт \"" + port + "\".");
        ...
        webTread = new Thread(() -> {
            try (final ServerSocket reportSocket = new ServerSocket(port, 0)) {
                ...
                log.info("Report server for QSystem started.");
            ...
            log.info("Отчетный вэбсервер системы 'Очередь' остановлен.");
    ...
    private final void go(ServerSocket reportSocket) {
        try {
