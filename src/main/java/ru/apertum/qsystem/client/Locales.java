...
public final class Locales {
    ...
    private Locales() {
        ...
        log.info("Languages are loading...");
        ...
        for (File list1 : list) {
            ...
            try (final InputStream in = settings.getClass().getResourceAsStream("/" + s + ".properties");
                ...
                log.error("Language description " + list1.getName() + " did NOT load. " + ex);
                continue;
            }
            log.debug("   Langusge: " + settings.getProperty("name") + " " + settings.getProperty("lng") + "_" + settings.getProperty("country"));
        ...
        locs.forEach(loc -> lngsWelcome.put(loc, config.getString(loc, "1")));
