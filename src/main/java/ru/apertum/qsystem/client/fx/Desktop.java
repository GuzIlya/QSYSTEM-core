...
public class Desktop extends Application {
    ...
    public static void main() {
        ...
        try {
            ...
            launch(Desktop.class, QConfig.cfg().getArgs());
    ...
    public void start(Stage stageDesktop) {
        ...
        try {
            ...
            throw new ServerException("Error desktop start.", e);
        }
        ...
        void start2() {
            ...
            if (!isActivate() && QConfig.cfg().isTerminal()) { // порт не занялся, т.к. в терминальном режиме, то нужно подсасывать мессаги из файла
                ...
                if (!new File(Uses.TEMP_FOLDER + File.separator + "pipe").exists()) {
                    if (!new File(Uses.TEMP_FOLDER + File.separator).mkdir()) {
                        log.error("Fail to make dir {}", Uses.TEMP_FOLDER + File.separator);
                    }
