...
import static ru.apertum.qsystem.client.QProperties.SECTION_RECEPTION;
 ...
 */
@SuppressWarnings({"squid:S1199", "squid:S1192", "squid:S1172", "squid:S1450", "squid:S1604", "squid:S1161", "squid:MaximumInheritanceDepth", "squid:ClassVariableVisibilityCheck"})
@Log4j2
public class FReception extends javax.swing.JFrame {
     ...
     * Локализация строк.
     */
    public static String locMess(String key) {
        return getLocaleMessage(key);
    }

    private final transient IClientNetProperty netProperty;
    ...
    private void init() {
        ...
        treeServices.addTreeSelectionListener(e -> serviceListChange());
        ...
        tablePreReg.addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                ...
                if (SwingUtilities.isRightMouseButton(e)) {
                    ...
                    int rowNumber = tablePreReg.rowAtPoint(point);
    ...
    private void showServiceInfo(QService service) {
        ...
        final StringBuilder servInfo = new StringBuilder(); //NOI18N
        for (QServiceLang sl : service.getLangs()) {
            servInfo.append(", ").append(sl.getLang()); //NOI18N
        }
        servInfo.append(servInfo.length() > 1 ? "[" + servInfo.substring(2) + "]" : ""); //NOI18N

        labelServiceInfo.setText("<html><body text=\"#336699\"> "
                + (service.getEnable() == 1 ? "" : "<font color=\"#FF0000\">!*** </font>")
                + servInfo + " " + getLocaleMessage("service.service") + service.getSeqId() + ": \"" + "<font color=\"#000000\">" + service.getName() + "\"    " + "</font>"
                + "<font color=\"#"
                ...
                + getLocaleMessage("service.description") + ": " + service.getDescription() //NOI18N //NOI18N
                + ";<br>" + getLocaleMessage("service.restrict_day_reg") + ": " + (service.getDayLimit() == 0
                ? getLocaleMessage("service.work_calendar.no")
                : service.getDayLimit()) //NOI18N
                + ";<br>" + getLocaleMessage("service.restrict_adv_reg") + " " + service.getAdvanceTimePeriod() + " " + getLocaleMessage("service.min")
                + ": " + service.getAdvanceLimit() //NOI18N
                + ";<br>  " + getLocaleMessage("service.restrict_adv_period") + ": " + service.getAdvanceLimitPeriod() //NOI18N
                + ";<br>"
                + (service.getInputRequired() ? getLocaleMessage("service.required_client_data") + ": \"" + service.getInputCaption().replaceAll("<[^>]*>", "")
                + "\"(" + service.getPersonDayLimit() + ")" : getLocaleMessage("service.required_client_data.not")) + ";<br>   "
                + (service.getResultRequired() ? getLocaleMessage("service.required_result") : getLocaleMessage("service.required_result.not")) + ";");
    ...
    private void preRegChange(boolean force) {
        ...
        labelPreDate.setText(Locales.getInstance().formatDdMmmmYyyy.format(preDate));
        ...
        final GridDayAndParams greed;
        try {
            greed = NetCommander.getPreGridOfDay(netProperty, preService.getId(), preDate, -1);
    ...
    private static class PreTableModel extends AbstractTableModel {
        ...
        private final transient RpcGetGridOfDay.GridDayAndParams greed;
    ...
    private class UsersMonModel extends AbstractTableModel {
        ...
        private final LinkedList<QUser> greed;
                ...
                case 1:
                    if (greed.get(rowIndex).getShadow() == null) {
                        ...
                        if (greed.get(rowIndex).getShadow().getStartTime() == null) {
                            ...
                            return "<HTML><SPAN STYLE='COLOR:" + (toolong ? "RED" : "GREEN") + "'>" + getLocaleMessage("Free2")
                                    + " " + mnt + getLocaleMessage("min.min") + "</span>";
                        } else {
    ...
    private class ServicesMonModel extends AbstractTableModel {
        ...
        private final LinkedList<ServiceInfo> greed;
    ...
    private void textFieldSerchServiceKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_textFieldSerchServiceKeyReleased
        ...
        for (Object object : ((ATreeModel) treeServices.getModel()).getNodes()) {
            ...
            if (flag) {
                ...
                if (ser == null || service.getId().equals(ser.getId())) {
                    flag = true;
                }
            }
        }
    ...
    private void buttonRefreshUserActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonRefreshUserActionPerformed
        if (listUsers.getLastVisibleIndex() != -1) {
            ...
            if (plan.getCustomer() != null) {
                log.trace("От сервера приехал кастомер, который обрабатывается юзером."); //NOI18N

                log.trace("Установливаем кастомера работающему клиенту и выводем его."); //NOI18N
                QCustomer customer = plan.getCustomer();
                final long min = (customer.getState() == CustomerState.STATE_INVITED || customer.getState() == CustomerState.STATE_INVITED_SECONDARY
                        ? 0
                ...
                String inputCaption = customer.getService().getInputCaption();
                if (inputCaption == null) {
                    inputCaption = ""; //NOI18N
                } else {
                    inputCaption = "<br>" + inputCaption + "<br>" + customer.getInputData(); //NOI18N
                }
                tempAll = tempAll + "<br>" + getLocaleMessage("working.with.client") + " " + textCust + " " + min + " " + getLocaleMessage("mints")
                        + "<br><b><span style='color:#000000'> " + getLocaleMessage("messages.service") + ": " + customer.getService().getName() + "<br>"
                        + getLocaleMessage("messages.priority") + ": " + getReceptionPriorityName(customer.getPriority().get()) + inputCaption + "</span></b>";
            } else {
                labelUserInfo.setIcon(null);
                if (plan.getShadow() == null) {
                    ...
                    tempAll = tempAll + "<br><span style='color:" + (toolong ? "red" : "green") + "'>" + getLocaleMessage("free3")
                            + " " + mnt + getLocaleMessage("min.dot") + "</span>";
                }
            }
    ...
    private String getReceptionPriorityName(int id) {
        switch (id) {
            case 0: {
                return locMess("messages.priority.low");
            }
            case 1: {
                return locMess("messages.priority.standart");
            }
            case 2: {
                return locMess("messages.priority.hi");
            }
            case 3: {
                return locMess("messages.priority.vip");
            }
            default: {
                return locMess("messages.priority.strange");
            }
        }
    }
    ...
    private void tablePreRegMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tablePreRegMouseClicked
        ...
        if (((evt == null) || (SwingUtilities.isLeftMouseButton(evt) && evt.getClickCount() > 1)) && tablePreReg.getSelectedRow() >= 0 && service != null && service.isLeaf()) {
            ...
            if (fc.getAcust().getAdvanceTime() == null) {
                final QProperty property = QProperties.get().getProperty(SECTION_RECEPTION, "allow-pre-registration");
                if (property != null && !property.getValueAsBool()) {
                    JOptionPane.showMessageDialog(this, "Forbidden by administration.", "Pre-registration", JOptionPane.WARNING_MESSAGE);
                    return;
                }
                if (0 == JOptionPane.showConfirmDialog(this, getLocaleMessage("make.pre.reg ") + fc + "?", getLocaleMessage("pre.reg"), JOptionPane.YES_NO_OPTION)) {
                    ...
                    if (service.getInputRequired()) {
                        inputData = (String) JOptionPane.showInputDialog(this, service.getInputCaption(), "***", 3, null, null, ""); //NOI18N
                        if (inputData == null) {
                            return;
                        }
                    }
                    ...
                    if (checkBoxPrintAdvTicket.isSelected()) {
                        new Thread(() -> FWelcome.printTicketAdvance(res, ((QService) treeServices.getModel().getRoot()).getTextToLocale(QService.Field.NAME))).start();
                    }
                    preRegChange(true);
                    JOptionPane.showMessageDialog(this, getLocaleMessage("admin.client_adv_dialog.msg_1") + " \"" + service.getName() + "\". "
                                    + getLocaleMessage("admin.client_adv_dialog.msg_2") + " \"" + res.getId() + "\".", getLocaleMessage("admin.client_adv_dialog.title"),
                ...
                if (0 == JOptionPane.showConfirmDialog(this, getLocaleMessage("clint.came") + fc + "?", getLocaleMessage("pre.reg.2"), JOptionPane.YES_NO_OPTION)) {
                    ...
                    final RpcStandInService.AdvResult res;
                    try {
                        res = NetCommander.standAndCheckAdvance(netProperty, fc.getAcust().getId());
                    } catch (QException ex) {
                        throw new ClientException("Error stand advance: " + ex.getLocalizedMessage() + "\n", ex);
                    }

                    if (res != null) {

                        if (res.getError() == null) {
                            log.info(getLocaleMessage("print.ticket"));
                            new Thread(() -> FWelcome.printTicket(res.getCustomer(), ((QService) treeServices.getModel().getRoot()).getTextToLocale(QService.Field.NAME))).start();
                            preRegChange(true);
                            JOptionPane.showMessageDialog(this, getLocaleMessage("admin.client_adv_dialog.msg_3"), getLocaleMessage("admin.client_adv_dialog.title"),
                                    JOptionPane.INFORMATION_MESSAGE);
                        } else {
                            JOptionPane.showMessageDialog(this, res.getError(), getLocaleMessage("admin.client_adv_dialog.title"), JOptionPane.ERROR_MESSAGE);
                        }
                    }
                }
            }
    ...
    private void listServicesForUserMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_listServicesForUserMouseClicked
        if (evt.getClickCount() > 1 && listServicesForUser.getSelectedIndex() != -1) {
            ...
            TreeNode[] nodes = ((DefaultTreeModel) treeServices.getModel()).getPathToRoot(((ATreeModel) treeServices.getModel())
    ...
    private void tableServicesMonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tableServicesMonMouseClicked
        if (evt.getClickCount() > 1 && tableServicesMon.getSelectedRow() != -1) {
            tabsPane.setSelectedComponent(panelServices);
            TreeNode[] nodes = ((DefaultTreeModel) treeServices.getModel()).getPathToRoot(((ATreeModel) treeServices.getModel())
                    .getById(((ServiceInfo) tableServicesMon.getModel().getValueAt(tableServicesMon.getSelectedRow(), 2)).getId()));
            TreePath path = new TreePath(nodes);
            treeServices.setSelectionPath(path);
            treeServices.setExpandsSelectedPaths(true);
            treeServices.scrollPathToVisible(path);
    ...
    private void buttonRemoveAdvanceCustomerActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonRemoveAdvanceCustomerActionPerformed
        if (tablePreReg.getSelectedRow() >= 0) {
            ...
            if (fc.getAcust().getAdvanceTime() != null
                    && 0 == JOptionPane.showConfirmDialog(this, "Удалить предварительную запись " + fc.getAcust().getId() + " ко времени " + fc + " безвозвратно?",
                    getLocaleMessage("pre.reg.2"), JOptionPane.YES_NO_OPTION)) {

                final JsonRPC20OK res;
                try {
                    res = NetCommander.removeAdvancedCustomer(netProperty, fc.getAcust().getId());
                } catch (QException ex) {
                    throw new ClientException("Error remove advanced customer: " + ex.getLocalizedMessage() + "\n", ex);
                }

                if (res != null) {

                    if (res.getResult() == 1) {// костыль. тут приедет ID отказа
                        log.info("Удалили предваоительного {} на {}", fc.getAcust().getId(), fc);

                        preRegChange(true);
                        JOptionPane.showMessageDialog(this, getLocaleMessage("admin.client_adv_remove.msg_3"), getLocaleMessage("admin.client_adv_dialog.title"),
                                JOptionPane.INFORMATION_MESSAGE);
                    } else {
                        JOptionPane.showMessageDialog(this, "Не найдена предварительная запись по номеру " + fc.getAcust().getId(),
    ...
    public static void main() {
        ...
        final IClientNetProperty netProperty = QConfig.cfg().getClientNetProperty();
        ...
        for (final IStartReception event : ServiceLoader.load(IStartReception.class)) {
            ...
            try {
                new Thread(() -> event.start(fReception)).start();
            } catch (Exception tr) {
    ...
    public boolean load() {
        ...
        try {
            ...
            Uses.closeSplash();
            log.error("Не загрузились стандарты. ", ex); //NOI18N
            return false;
        }
        ...
        try {
            ...
            Uses.closeSplash();
            log.error("Не загрузились пользователи. ", ex); //NOI18N
        ...
        try {
            ...
            QServiceTree.sailToStorm(servs.getRoot(), service -> slist.add((QService) service));

            for (QService serv : slist) {
                serv.getChildren().stream().map(service -> {
                    service.setParent(serv);
                    return service;
                }).forEach(service1 -> service1.setParentId(serv.getId()));
            }
            ...
            Uses.closeSplash();
            log.error("Не загрузились услуги. ", ex); //NOI18N
            return false;
        }
        ...
        try {
            ...
            int amt = 0;
            amt = srvs.stream().map(ServiceInfo::getCountWait).reduce(amt, Integer::sum);
            ...
            log.error("Сервер ответил на запрос о состоянии: \"" + ex + "\""); //NOI18N
    ...
    public void setInLine() {
        ...
        if (service != null && service.isLeaf()) {
            ...
            String inputData = null;
            if (service.getInputRequired()) {
                inputData = (String) JOptionPane.showInputDialog(this, service.getInputCaption().replaceAll("<[^>]*>", ""), "***", 3, null, null, ""); //NOI18N
            ...
            JOptionPane.showMessageDialog(this, getLocaleMessage("admin.print_ticket.title") + " \"" + service.getName() + "\". "
                            + getLocaleMessage("admin.print_ticket.title_1") + " \"" + customer.getFullNumber() + "\".",
    ...
    private class QCustomer2 implements IidGetter {
        ...
        public String toString() {
            ...
            return "<html>" + nom + ".  " + customer.getFullNumber() + (customer.getPostponedStatus().isEmpty() ? "" : " " + customer.getPostponedStatus()
                    ...
                    + (customer.getInputData().isEmpty() ? "" : "  <u>" + customer.getInputData()) + "</u>    "
                    + /*txt("messages.priority") + ": " +*/ priority + "  "
                    + "<span style='color:" + (min <= standards.getWaitMax() ? "green" : "red")
                    + "'>" + " - " + getLocaleMessage("waiting") + " " + min + " " + getLocaleMessage("mints2") + "</span>";
        }
            ...
            if (name != null) {
                for (int i = 0; i < Uses.getPrioritysWord().size(); i++) {
                    if (name.equals(Uses.getPrioritysWord().get(i))) {
                        try {
                            JOptionPane.showMessageDialog(this, NetCommander.setCustomerPriority(netProperty, i, cus.customer.getFullNumber()),
                                    getLocaleMessage("admin.action.change_priority.title"), JOptionPane.INFORMATION_MESSAGE);
                        } catch (QException ex) {// вывод исключений
                            throw new ClientException(locMes("command_error") + ": " + ex.getLocalizedMessage(), ex);
                        }
                        refreshLines();
    ...
    /**
     * Установим любой приоритет.
     */
    @Action
    public void setAnyPriority() {
        final String num = (String) JOptionPane.showInputDialog(this, getLocaleMessage("admin.action.change_priority.num.message"),
    ...
    public void checkAnyTicket() {
        ...
        if (num != null) {
            ...
            Uses.setLocation(f);
            final RpcGetTicketHistory.TicketHistory ticketHistory;
            try {
                ticketHistory = NetCommander.checkCustomerNumber(netProperty, num);
            } catch (QException ex) {// вывод исключений
                throw new ClientException(locMes("command_error") + ": " + ex.getLocalizedMessage(), ex);
            }
            f.setInfo(ticketHistory.getInfo());
            StringBuilder sb = new StringBuilder("<html>");
            ticketHistory.getCusts().stream().forEach(c -> sb.append(c).append("<br>"));
    ...
    public void changeStatusForPostponed() {
        if (listPostponed.getSelectedIndex() != -1) {
            ...
            String status = (String) JOptionPane.showInputDialog(this, getLocaleMessage("resultwork.dialog.caption"),
                    getLocaleMessage("resultwork.dialog.title"), JOptionPane.QUESTION_MESSAGE, null, getResults(), null);
            if (status == null) {
                return;
            }
            try {
