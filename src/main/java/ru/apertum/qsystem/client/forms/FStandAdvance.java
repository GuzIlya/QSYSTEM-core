...
public class FStandAdvance extends javax.swing.JDialog {
    ...
    private transient RpcStandInService.AdvResult result = null;
     ...
     * @return описание результата предварительной записи, по сути это номерок. если null, то отказались от предварительной записи
     */
    public static RpcStandInService.AdvResult showAdvanceStandDialog(Frame parent, boolean modal, INetProperty netProperty, boolean fullscreen, int delay) {
        FStandAdvance.delay = delay;
        log.info("Ввод кода предварительной записи");
        if (standAdvance == null) {
            standAdvance = new FStandAdvance(parent, modal);
            standAdvance.setTitle(getLocaleMessage("dialog.input_code"));
        }
        standAdvance.result = null;

        if (Uses.firstMonitor.getDefaultConfiguration().getBounds().width < standAdvance.getWidth()) {
            standAdvance.setSize(Uses.firstMonitor.getDefaultConfiguration().getBounds().width - 40, standAdvance.getHeight());
        }
        ...
        return standAdvance.result;
    }

    private String defaultKod = getLocaleMessage("dialog.default_code");
    /**
    ...
    private void initComponents() {
        ...
        panelAll = new QPanel(WelcomeParams.getInstance().getBackgroundImg("standadvance"));
        ...
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance().getContext().getResourceMap(FStandAdvance.class);
        ...
        panelAdvKodLayout.setHorizontalGroup(
                panelAdvKodLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                        .addGroup(panelAdvKodLayout.createSequentialGroup()
                                .addContainerGap()
                                .addComponent(labelKod, javax.swing.GroupLayout.DEFAULT_SIZE, 856, Short.MAX_VALUE)
                                .addContainerGap())
        );
        ...
        panelMainLayout.setHorizontalGroup(
                panelMainLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                        ...
                        .addGroup(panelMainLayout.createSequentialGroup()
                                .addContainerGap()
                                .addComponent(panelButtonsNumeric, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                                .addContainerGap())
        );
    ...
    private void changeTextToLocale() {
        ...
        jButton1.setText(resourceMap.getString("jButton1.text")); // NOI18N
        jButton2.setText(resourceMap.getString("jButton2.text")); // NOI18N
        defaultKod = getLocaleMessage("dialog.default_code");
        labelKod.setText(defaultKod);
        jLabel2.setText(resourceMap.getString("jLabel2.text")); // NOI18N
    }
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if (labelKod.getText().length() != 0 && !defaultKod.equals(labelKod.getText())) {
            if (clockBack.isActive()) {
                clockBack.stop();
            }
            try {
                ...
                log.error("Error stand advance.", ex);
            }
            setVisible(false);
        }
    ...
    private void buttonClickNumeric(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonClickNumeric
        ...
        if ("".equals(evt.getActionCommand())) {
            // удаление
            if (defaultKod.equals(labelKod.getText()) || "".equals(labelKod.getText()) || labelKod.getText().length() == 1) {
                labelKod.setText(defaultKod);
            } else {
            ...
            // добавление цифры
            if (defaultKod.equals(labelKod.getText())) {
                labelKod.setText("");
            }
