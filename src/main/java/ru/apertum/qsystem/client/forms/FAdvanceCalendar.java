...
public class FAdvanceCalendar extends javax.swing.JDialog {
    ...
    private QAdvanceCustomer result = null;
    ...
    public static QAdvanceCustomer showCalendar(Frame parent, boolean modal, INetProperty netProperty, QService service, boolean fullscreen, int delay,
                                                long advCustomer, String inputData, String comments) {
        ...
        log.info("Выбор времени для предварительной записи");
    ...
    private void initComponents() {
        ...
        qPanel1 = new QPanel(WelcomeParams.getInstance().getBackgroundImg("advcalendar"));
    ...
    private boolean showWeek(Date firstWeekDay) {
        ...
        final GridAndParams res;
        try {
            res = NetCommander.getGridOfWeek(netProperty, service.getId(), this.firstWeekDay, advancedCustomer);
        } catch (Exception ex) {
            throw new ClientException(locMes("bad_response") + "\n" + ex.toString(), ex);
        }
        if (res.getSpError() != null) {
            log.error(res.getSpError());
    ...
    private void showWeek(GridAndParams res, GregorianCalendar gc) {
        ...
        log.debug("Предварительная запись с " + res.getStartTime() + " до " + res.getFinishTime());
        ...
        if (WelcomeParams.getInstance().advanceCloseButtonEmpty) {
            buttonClose.setText("");
            Container parent = buttonClose.getParent();
            parent.remove(buttonClose);
    ...
    private void printDayWeek(JPanel panel, GridAndParams res, int weekDay) {
        ...
        for (Date dd : res.getTimes()) {
            ...
            final boolean f = listSoFare(res, gcClient, gcNow);

            if (ii == weekDay && f && gc.getTime().after(gcNow.getTime())) {
    ...
    private boolean listSoFare(GridAndParams res, GregorianCalendar gcClient, GregorianCalendar gcNow) {
        // проверим не отлистал ли пользователь слишком далеко, куда уже нельзя
        boolean f = true;
        int per = 0;
    ...
    private void printCaptionWeek(JLabel label, int day, GregorianCalendar gc) {
        ...
        SimpleDateFormat formatDdMmmmYyyy = new SimpleDateFormat("dd MMMM yyyy", Locales.getInstance().getLangCurrent());
        if (now.get(GregorianCalendar.DAY_OF_MONTH) == gc.get(GregorianCalendar.DAY_OF_MONTH)
            ...
            if (WelcomeParams.getInstance().advanceDaysHeaderToday.isEmpty()) {
                txt = "<html><p align=center><span style='font-size:22.0pt;color:red'>" + getNameWeekDay(gc.getTime())
                        + "<br/></span><span style='font-size:22.0pt;color:red'>" + formatDdMmmmYyyy.format(gc.getTime());
            } else {
                txt = String.format(WelcomeParams.getInstance().advanceDaysHeaderToday, getNameWeekDay(gc.getTime()), formatDdMmmmYyyy.format(gc.getTime()));
            }
            label.setText(txt);
        } else {
            final String txt;
            if (WelcomeParams.getInstance().advanceDaysHeader.isEmpty()) {
                txt = "<html><p align=center><span style='font-size:22.0pt;color:blue'>" + getNameWeekDay(gc.getTime())
                        + "<br/></span><span style='font-size:18.0pt;color:black'>" + formatDdMmmmYyyy.format(gc.getTime());
            } else {
                txt = String.format(WelcomeParams.getInstance().advanceDaysHeader, getNameWeekDay(gc.getTime()), formatDdMmmmYyyy.format(gc.getTime()));
            }
            label.setText(txt);
        }
    }
    ...
    private String getNameWeekDay(Date date) {
        ...
        switch (gc.get(GregorianCalendar.DAY_OF_WEEK)) {
            case GregorianCalendar.MONDAY:
                dayName = getLocaleMessage("calendar.day.monday");
                break;
            case GregorianCalendar.TUESDAY:
                dayName = getLocaleMessage("calendar.day.tuesday");
                break;
            case GregorianCalendar.WEDNESDAY:
                dayName = getLocaleMessage("calendar.day.wednesday");
                break;
            case GregorianCalendar.THURSDAY:
                dayName = getLocaleMessage("calendar.day.thursday");
                break;
            case GregorianCalendar.FRIDAY:
                dayName = getLocaleMessage("calendar.day.friday");
                break;
            case GregorianCalendar.SATURDAY:
                dayName = getLocaleMessage("calendar.day.saturday");
                break;
            case GregorianCalendar.SUNDAY:
                dayName = getLocaleMessage("calendar.day.sunday");
                break;
            default:
