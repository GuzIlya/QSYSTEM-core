...
public class FServiceChangeDialod extends javax.swing.JDialog {
     ...
     * @param scheduleModel для выбора.
     * @param calendarModel для выбора.
     */
    public static void changeService(Frame parent, boolean modal, QService service, ComboBoxModel scheduleModel, ComboBoxModel calendarModel) {
        log.info("Редактирование услуги \"" + service.getName() + "\"");
        ...
        serviceChangeDialod.setTitle(txt("dialog.title") + "  ID " + service.getId());
    ...
    private void loadService(QService service) {
        ...
        checkBoxInputRequired.setSelected(service.getInputRequired());
        textFieldInputCaption.setText(service.getInputCaption());
        ...
        checkBoxResultRequired.setSelected(service.getResultRequired());
    ...
    private void loadSoundTemplate() {
        ...
        cbSound.setSelected(parts != null && parts.length != 0 && parts[0].startsWith("1"));
        if (parts != null) {
            if (parts[0].length() > 1) {
    ...
    private void saveService() {
        if ("".equals(textAreaButtonCaption.getText())) {
            throw new ClientException(txt("dialog.message1"));
        }
        if (textAreaButtonCaption.getText().length() > 2500) {
            throw new ClientException(txt("dialog.message2"));
        }
        if (textAreaInfoHtml.getText().length() > 100000) {
            throw new ClientException(txt("dialog.message3"));
        }
        if (textAreaTextPrint.getText().length() > 100000) {
            throw new ClientException(txt("dialog.message4"));
        }
        ...
        if (!f.isEmpty() && JOptionPane.showConfirmDialog(null, txt("dialog.message5"), txt("confirm"), JOptionPane.OK_CANCEL_OPTION) == JOptionPane.CANCEL_OPTION) {
            throw new ServerException(txt("dialog.message5"));
        }
        ...
        service.setInputRequired(checkBoxInputRequired.isSelected());
        service.setInputCaption(textFieldInputCaption.getText());
        service.setInputedAsExt(cbRequaredAsColumnExt.isSelected());
        service.setResultRequired(checkBoxResultRequired.isSelected());
    ...
    private void loadServiceLang(QServiceLang serviceLng) {
        ...
        textFieldInputCaption.setText(serviceLng.getInputCaption());
    ...
    private void saveServiceLang() {
        ...
        serviceLng.setInputCaption(textFieldInputCaption.getText());
    ...
    private void initComponents() {
        ...
        comboBoxEnabled.setModel(new javax.swing.DefaultComboBoxModel(new String[]{"Активна", "Неактивна", "Невидима", "Только предварительная запись",
                "Заглушка", "Нет предварительной записи", "Рулон талонов"}));
        comboBoxEnabled.setName("comboBoxEnabled"); // NOI18N
        comboBoxEnabled.setModel(new javax.swing.DefaultComboBoxModel(new String[]{txt("active"), txt("notActive"), txt("invisible"),
                txt("forPrereg"), txt("forDummy"), txt("noPrereg"), txt("roll")}));
