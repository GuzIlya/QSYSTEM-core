...
/**
 * Диалог. Изменить языков для услуги.
 *
...
public class FServiceLangList extends javax.swing.JDialog {
    ...
    public static void changeServiceLangList(Frame parent, boolean modal, final QService service) {
        log.info("Редактирование языковых локаций \"" + service.getName() + "\"");
    ...
    private void initComponents() {
        ...
        javax.swing.ActionMap actionMap = org.jdesktop.application.Application.getInstance().getContext().getActionMap(FServiceLangList.class, this);
        ...
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance().getContext().getResourceMap(FServiceLangList.class);
    ...
    /**
     * отредактируем тексты для языка.
     */
    @Action
    public void editLng() {
        if (listLng.getSelectedIndex() != -1) {
    ...
    private void edtLng(QServiceLang lng) {
        ...
        while (flag) {
            ...
            lngName = (String) JOptionPane.showInputDialog(this, getLocaleMessage("LngList.lng_dialog.caption"), getLocaleMessage("LngList.lng_dialog.caption"),
                    3, null, null, lngName);
            if (lngName == null) {
                return;
            }
            for (QServiceLang qb : service.getLangs()) {
                if (qb.getLang().equalsIgnoreCase(lngName) && !lngName.equalsIgnoreCase(old)) {
                    JOptionPane.showConfirmDialog(this, getLocaleMessage("LngList.lng_dialog.err0.message"), getLocaleMessage("LngList.lng_dialog.caption"),
                            JOptionPane.DEFAULT_OPTION, JOptionPane.ERROR_MESSAGE);
                    return;
                }
            }
            if ("".equals(lngName)) {
                JOptionPane.showConfirmDialog(this, getLocaleMessage("LngList.lng_dialog.err1.message"), getLocaleMessage("LngList.lng_dialog.caption"),
                        JOptionPane.DEFAULT_OPTION, JOptionPane.ERROR_MESSAGE);
            } else {
                if (lngName.indexOf('\"') != -1) {
                    JOptionPane.showConfirmDialog(this, getLocaleMessage("LngList.lng_dialog.err2.message"), getLocaleMessage("LngList.lng_dialog.caption"),
                            JOptionPane.DEFAULT_OPTION, JOptionPane.ERROR_MESSAGE);
                } else {
                    if (lngName.length() > 75) {
                        JOptionPane.showConfirmDialog(this, getLocaleMessage("LngList.lng_dialog.err3.message"), getLocaleMessage("LngList.lng_dialog.caption"),
                                JOptionPane.DEFAULT_OPTION, JOptionPane.ERROR_MESSAGE);
                    } else {
                        flag = false;
                    }
        ...
        if (lng == null) {
            ...
            log.debug("Редактируем язык \"" + lngName + "\"");
            item = lng;
        }
