...
public class FMedCheckIn extends javax.swing.JDialog {
    ...
    private QAuthorizationCustomer result = null;
     ...
     *
     * @param port порт.
     * @throws Exception может упасть.
     */
    public void setPort(final ISerialPort port) throws Exception {//NOSONAR
        port.bind(new IReceiveListener() {
            ...
            public void actionPerformed(SerialPortEvent event, byte[] data) {
                ...
                log.debug("Приняли сообщение в СОМ-порт \"" + result + "\"");
    ...
    public static QAuthorizationCustomer showMedCheckIn(Frame parent, boolean modal, INetProperty netProperty, boolean fullscreen, ISerialPort port) {
        log.info("Показать форму идентификации клиента для предварительной записи");
        if (medCheckIn == null) {
            medCheckIn = new FMedCheckIn(parent, modal);
            medCheckIn.setTitle("Идентификация клиента.");
        }
        medCheckIn.result = null;
        ...
        if (port != null && port.getListeners().isEmpty()) {
            try {
                ...
                log.error("Ошибка при присвоении порту листенера.");
            }
        }
        medCheckIn.setVisible(true);
        return medCheckIn.result;
    }
    ...
    /**
     * ЗАблокируем диалогом.
     *
     * @param isBlock да или нет.
     */
    ...
    private void setFordFocuseEdit(final JComponent e) {
        ...
        java.awt.EventQueue.invokeLater(e::requestFocus);
    }
    ...
    private void buttonEnterActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonEnterActionPerformed
        setFordFocuseEdit(textFieldNumber);
        if ("".equals(textFieldNumber.getText().trim()) || !buttonEnter.isVisible()) {
            return;
        }
        //Послать запрос на идентификацию клиента и отработать результат
        QAuthorizationCustomer res = null;
        try {
            res = NetCommander.getClientAuthorization(netProperty, textFieldNumber.getText().trim());
        } catch (QException ex) {
            log.error("Error checking client \"" + textFieldNumber.getText().trim() + "\"", ex);
        }
        // Шлем отказ
        ...
        if (res == null) {
            log.debug("Не идентифицирован клиент по номеру \"" + textFieldNumber.getText() + "\"");
            FTimedDialog.showTimedDialog(null, true, notFound, 5000);
        } else {
            log.debug("Клиент опознан, предлагаем выбрать услугу");
            result = res;
            closeMedCheckIn();
        }
