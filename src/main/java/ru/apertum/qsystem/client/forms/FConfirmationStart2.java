...
/**
 * Диалог подтверждения стоять в очереди если очередб не пусая.
 *
...
public class FConfirmationStart2 extends javax.swing.JDialog {
    ...
    /**
     * Открыть диалог подтверждения стоять в очереди и получить результат выбора.
     *
     * @param owner родительская форма.
     * @param count количество уже стоящих ожиающих перед новым.
     * @return встает или не встает в очередь.
     */
    ...
    private void initComponents() {
        ...
        jPanel1 = new QPanel(WelcomeParams.getInstance().getBackgroundImg("start"));
    ...
    private void changeTextToLocale(int count) {
        final org.jdesktop.application.ResourceMap resourceMap =
                org.jdesktop.application.Application.getInstance(ru.apertum.qsystem.QSystem.class).getContext().getResourceMap(FConfirmationStart.class);
        ...
        labelInfo.setIcon(new File(WelcomeParams.getInstance().confirmationStartImg).exists()
                ? new ImageIcon(WelcomeParams.getInstance().confirmationStartImg)
                : new javax.swing.ImageIcon(getClass().getResource("/ru/apertum/qsystem/client/forms/resources/vopros.png")));
