...
public class FInfoDialogWeb extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        org.jdesktop.application.ResourceMap resourceMap = org.jdesktop.application.Application.getInstance().getContext().getResourceMap(FInfoDialogWeb.class);
    ...
    static class Browser extends Region {
        ...
        public Browser() {
            ...
            try {
                webView.getEngine().setUserStyleSheetLocation(this.getClass().getResource("/ru/apertum/qsystem/fx/css/scroll.css").toString());
            } catch (Exception ex) {
                log.error(ex);
            }

            webView.setOnTouchPressed((TouchEvent event) -> {
                if (infoDialogWeb.clockBack.isActive()) {
                    infoDialogWeb.clockBack.stop();
                }
                infoDialogWeb.clockBack.start();
