...
public class AddrProp {
    ...
    private void addAddrsFromLine(String line) {
        if (!line.isEmpty() && !line.startsWith("#")) {
            ...
            if (ss.length != 2) {
                log.error(ADDR_FILE.getAbsolutePath() + " contaned an error, line: \"" + line + "\"");
            } else {
                ...
                if ((ssl.length == 1 && Uses.isInt(ss[0])) || (ssl.length == 2 && Uses.isInt(ss[0]) && Uses.isInt(ssl[1]))) {
                    ...
                    log.trace(ADDR_FILE.getAbsolutePath() + " Read line: \"" + line + "\"");
                } else {
                    log.error(ADDR_FILE.getAbsolutePath() + " contaned an error: \"" + line + "\" value \"" + Arrays.toString(ssl) + "\" is bad.");
                }
            }
        }
    }
