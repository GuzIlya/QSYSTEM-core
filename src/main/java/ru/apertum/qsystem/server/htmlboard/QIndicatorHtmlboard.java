...
public class QIndicatorHtmlboard implements IIndicatorBoard {
    ...
    protected void initIndicatorBoard() {
        ...
        if (indicatorBoard == null) {
            ...
            indicatorBoard.toPosition(QConfig.cfg().isDebug());
    ...
    public QIndicatorHtmlboard() {
        log.info("Создание HTML табло для телевизоров или мониторов. Шаблон табло в \"" + CONTENT_FILE_PATH + "\"");
    }
        ...
        elist.forEach(elem -> {
            switch (elem.attributeValue("Наименование")) {
                ...
                case "need_reload":
                    ...
                    break;
                default:
                    log.error("Параметер для HTML очень странный: " + elem.attributeValue("Наименование"));
            }
    ...
    public void showBoard() {
        log.trace("Показываем HTML табло");
        initIndicatorBoard();
        log.trace("HTML табло должно уже показаться.");
    }
    ...
    public synchronized void close() {
        log.trace("Закрываем HTML табло");
        if (indicatorBoard != null) {
    ...
    @Override
    public void refresh() {
        log.trace("Обновляем HTML табло");
        close();
        indicatorBoard = null;
        initIndicatorBoard();
    }
    ...
    public void customerStandIn(QCustomer customer) {
        log.trace("Поставили кастомера на BS табло");
        if (indicatorBoard != null) {
    ...
    public synchronized void inviteCustomer(QUser user, QCustomer customer) {
        log.trace("Приглшием кастомера на BS табло");
        if (indicatorBoard != null) {
            if (HtmlBoardProps.getInstance().isNeedReload()) {
                indicatorBoard.loadContent(prepareContent(CONTENT_FILE_PATH));
            }
    ...
    public void workCustomer(QUser user) {
        log.trace("Работа с кастомером на BS табло");
        if (indicatorBoard != null) {
            if (HtmlBoardProps.getInstance().isNeedReload()) {
                indicatorBoard.loadContent(prepareContent(CONTENT_FILE_PATH));
            }
    ...
    public void killCustomer(QUser user) {
        log.trace("Убираем кастомера на BS табло");
        if (indicatorBoard != null) {
            if (HtmlBoardProps.getInstance().isNeedReload()) {
                indicatorBoard.loadContent(prepareContent(CONTENT_FILE_PATH));
            }
