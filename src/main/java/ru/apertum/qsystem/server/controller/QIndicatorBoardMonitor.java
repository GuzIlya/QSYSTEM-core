...
public class QIndicatorBoardMonitor extends AIndicatorBoard {
    ...
    protected void initIndicatorBoard() {
        if (indicatorBoard == null) {
            ...
            if (indicatorBoard == null) {
                log.warn("Табло не демонстрируется. Отключено в настройках.");
                return;
            }
            try {
    ...
    public QIndicatorBoardMonitor() {
        log.info("Создание табло для телевизоров или мониторов.");
    }
    ...
    public Element getConfig() {
        ...
        if (boardFile.exists()) {
            try {
                ...
                log.error("Невозможно прочитать файл конфигурации главного табло. " + ex.getMessage());
                return DocumentHelper.createElement("Ответ");
            }
        } else {
            log.warn("Файл конфигурации главного табло \"" + configFile + "\" не найден. ");
            return DocumentHelper.createElement("Ответ");
        }
    }
