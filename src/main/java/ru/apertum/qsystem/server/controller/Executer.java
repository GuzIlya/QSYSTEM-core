...
import ru.apertum.qsystem.extra.IServerTaskWrapper;
...
import static ru.apertum.qsystem.common.cmd.JsonRPC20Error.ErrorMsg.ADVANCED_NOT_FOUND;
import static ru.apertum.qsystem.common.cmd.JsonRPC20Error.ErrorMsg.POSTPONED_NOT_FOUND;
import static ru.apertum.qsystem.common.cmd.JsonRPC20Error.ErrorMsg.REQUIRED_CUSTOMER_NOT_FOUND;
...
public final class Executer {
    ...
    private static final String REG_EXP_FORN = "[^\\p{L}+\\d]";
    ...
    private Executer() {
        ...
        for (final ITask task : ServiceLoader.load(ITask.class)) {
            log.info("Load extra task: " + task.getDescription());
            try {
                tasks.put(task.getName(), task);
            } catch (Exception tr) {
                log.error("Вызов SPI расширения ITask завершился ошибкой. Описание: " + tr);
            }
        }
    }
    ...
    /**
     * Базовый класс обработчиков заданий. сам себя складывает в HashMap[String, ATask] tasks. метод process исполняет задание.
     *
     * @author Evgeniy Egorov
     */
    public class Task implements ITask {
        ...
        /**
         * Создание таска вместе с передачей имени. По имени потом находится таска.
         *
        ...
        public AJsonRPC20 process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            log.debug("Processing: \"" + name + "\" " + ipAdress);
    ...
    class AddCustomerTask extends Task {
        ...
        public RpcStandInService process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            try {
                ...
                final boolean asNumber = service.getInputRequired() && service.getInputedAsNumber() > 0 && cmdParams.textData != null && cmdParams.textData.length() > 0;
                ...
                if (asNumber) {
                    customer.setPrefix(cmdParams.textData.length() >= service.getInputedAsNumber()
                            ? cmdParams.textData.substring(0, service.getInputedAsNumber())
                            : cmdParams.textData);
                }
                ...
                customer.setInputData(cmdParams.textData);
            ...
            log.trace("С приоритетом " + customer.getPriority().get() + " К услуге \"" + cmdParams.serviceId + "\" -> " + service.getPrefix()
                    + '\'' + service.getName() + '\'' + " " + ipAdress);
    ...
    final Task addCustomerTaskComplex = new Task(Uses.TASK_STAND_COMPLEX) {
        ...
        public RpcStandInService process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            for (final ISelectNextService event : ServiceLoader.load(ISelectNextService.class)) {
                log.info("Вызов SPI расширения. Описание: " + event.getDescription() + " " + ipAdress);
                try {
                    serviceID = event.select(null, null, cmdParams.complexId).getId();
                } catch (Exception tr) {
                    log.error("Вызов SPI расширения ISelectNextService завершился ошибкой. Описание: " + tr + " " + ipAdress);
                }
            }
            ...
            txtCustomer.getResult().getCustomer().setComplexId(cmdParams.complexId);
            return txtCustomer;
        }
    };
    /**
    ...
    final Task inviteCustomerTask = new Task(Uses.TASK_INVITE_NEXT_CUSTOMER) {
        ...
        class MyRun implements Runnable {
            ...
            public void run() {
                ...
                if (user.getCustomer() != null && (user.getCustomer().getState() == CustomerState.STATE_WAIT
                    ...
                    try {
                        SoundPlayer.inviteClient(user.getCustomer().getService(), user.getCustomer(),
                                user.getCustomer().getPrefix() + (user.getCustomer().getNumber() < 1 ? "" : user.getCustomer().getNumber()), user.getPoint(), isFrst);
                    } catch (Exception ex) {
                        log.error("Error of sound player!", ex);
                    }
        ...
        public synchronized RpcInviteCustomer process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            final boolean isRecall = user.getCustomer() != null
                    && (CustomerState.STATE_INVITED.equals(user.getCustomer().getState()) || CustomerState.STATE_INVITED_SECONDARY.equals(user.getCustomer().getState()));

            // есть ли у юзера вызванный кастомер? Тогда поторный вызов
            if (isRecall) {
                user.getCustomer().upRecallCount(); // еще один повторный вызов
                log.debug("Повторный вызов " + user.getCustomer().getRecallCount()
                        + " кастомера №" + user.getCustomer().getPrefix() + user.getCustomer().getNumber() + " пользователем " + cmdParams.userId + " " + ipAdress);
                ...
                if (ServerProps.getInstance().getProps().getLimitRecall() != 0 && user.getCustomer().getRecallCount() > ServerProps.getInstance().getProps().getLimitRecall()) {
                    log.debug("Превышение повторных вызовов для кастомера №"
                            + user.getCustomer().getPrefix() + user.getCustomer().getNumber() + " пользователем " + cmdParams.userId + " " + ipAdress);
                    //Удалим по неявки
                    killCustomerTask.process(cmdParams, ipAdress, ip);
                } else {
                    // кастомер переходит в состояние в котором был в такое и переходит.
                    user.getCustomer().setState(user.getCustomer().getState(), -2L);
            ...
            try {
                ...
                if ((cmdParams.textData != null && !cmdParams.textData.isEmpty()) || cmdParams.customerId != null) {
                    if (cmdParams.customerId == null) {
                        final String num = cmdParams.textData.replaceAll(REG_EXP_FORN, "");
                        log.debug("Warning! Corruption was detected! \"" + num + "\"" + " " + ipAdress);
                        ...
                        log.debug("Warning! Smart corruption was detected! \"" + cmdParams.customerId + "\"" + " " + ipAdress);
                        for (QService service : QServiceTree.getInstance().getNodes()) {
                            if ((customer = service.gnawOutCustomerById(cmdParams.customerId)) != null) {
                                log.debug("Warning! Clever corruption was detected! \"" + cmdParams.customerId + "\"" + " " + ipAdress);
                                break;
                            }
                        }
                        if (customer == null) {
                            ...
                            if ((customer = QPostponedList.getInstance().getById(cmdParams.customerId)) != null
                                ...
                                log.debug("Warning! Clever corruption was detected in postponed! \"" + cmdParams.customerId + "\"" + " " + ipAdress);
                            } else {
                                customer = null;
                            }
                        }
    ...
    final Task invitePostponedTask = new Task(Uses.TASK_INVITE_POSTPONED) {
        ...
        public synchronized AJsonRPC20 process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            try {
                // просигналим звуком
                SoundPlayer.inviteClient(customer.getService(), customer,
                        user.getCustomer().getPrefix() + (user.getCustomer().getNumber() < 1 ? "" : user.getCustomer().getNumber()), user.getPoint(), true);
                // сохраняем состояния очередей.
                QServer.savePool();
    ...
    /**
     * Проверим возможность встать в очередь.
    ...
    final Task aboutTask = new Task(Uses.TASK_ABOUT_SERVICE) {
        ...
        public RpcGetServiceState process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            if (srv.isLimitPerDayOver()) {
                log.warn("Услуга  \"" + srv.getName() + "\" не обрабатывается исходя из достижения лимита возможной обработки кастомеров в день." + " " + ipAdress);
            ...
            if (srv.getSchedule() == null
                ...
                if (srv.getSchedule() == null) {
                    log.warn("Если нет расписания, то отказ по расписанию." + " " + ipAdress);
                } else {
                    if (QCalendarList.getInstance().getById(1).checkFreeDay(day)) {
                        log.warn("Если выходной то отказ по расписанию." + " " + ipAdress);
                    } else {
                        log.warn("Если нет календаря и выходной то отказ по расписанию." + " " + ipAdress);
                    }
                }
                ...
                if (!(interval.start == null || interval.finish == null)) {
                    final int h = gcDay.get(GregorianCalendar.HOUR_OF_DAY);
                    final int m = gcDay.get(GregorianCalendar.MINUTE);
                    gcDay.setTime(interval.start);
                    final int sh = gcDay.get(GregorianCalendar.HOUR_OF_DAY);
                    final int sm = gcDay.get(GregorianCalendar.MINUTE);
                    gcDay.setTime(interval.finish);
                    final int eh = gcDay.get(GregorianCalendar.HOUR_OF_DAY);
                    final int em = gcDay.get(GregorianCalendar.MINUTE);
                    if (!(sh * 60 + sm <= h * 60 + m && h * 60 + m <= eh * 60 + em) && (!((sh == eh) && (sm == em)))) {
                        log.warn("Если текущее время не попадает в рабочий интервал то отказ по расписанию. "
                                + sh + "." + sm + " < " + h + "." + m + " < " + eh + "." + em + " " + ipAdress);
                        min = Uses.LOCK_FREE_INT;
                    }
                } else {
                    log.warn("Если в этот день не определено начало или конец то отказ по расписанию." + (interval.start == null ? "start == null" : "end == null") + " " + ipAdress);
                    min = Uses.LOCK_FREE_INT;
                }
            }
            // Если не работаем, то отправим ответ и прекратим выполнение
            if (min == Uses.LOCK_FREE_INT) {
                log.warn(" Услуга \"" + cmdParams.serviceId + "\" не обрабатывается исходя из рабочего расписания." + " " + ipAdress);
            ...
            if (min == Uses.LOCK_INT) {
                log.warn("Услуга   \"" + cmdParams.serviceId + "\" не обрабатывается ни одним пользователем." + " " + ipAdress);
     ...
     */
    final Task getServerState = new Task(Uses.TASK_SERVER_STATE) {
    ...
    final Task getSelfServicesTask = new Task(Uses.TASK_GET_SELF_SERVICES) {
        ...
        public RpcGetSelfSituation process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            for (QPlanService planService : user.getPlanServices()) {
                ...
                servs.add(new RpcGetSelfSituation.SelfService(service, service.getCountCustomers(), planService.getCoefficient(),
                        planService.getFlexibleCoef(), planService.getFlexibleInvitation()));
            }
            final long userHash = getUserHash(user); // это хэш всей обстановки по услуге для пользователя.
            final Long hash = hashState.get(cmdParams.userId);
            if (hash == null) {
                hashState.put(cmdParams.userId, userHash);
            } else {
        ...
        private long getUserHash(QUser user) {
            ...
            if (user.getParallelCustomers() != null) {
                for (Map.Entry<Long, QCustomer> entry : user.getParallelCustomers().entrySet()) {
                    stateH = stateH + entry.getValue().getId();
                }
            }
            return stateH;
        }
     ...
     */
    public final Map<Long, Long> hashState = new HashMap<>();
    /**
    ...
    final Task getCheckSelfTask = new Task(Uses.TASK_GET_SELF_SERVICES_CHECK) {
        ...
        public synchronized RpcGetBool process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            if (!QSessions.getInstance().check(cmdParams.userId, ipAdress, ip)) {
                log.debug(cmdParams.userId + " ACCESS_DENY from " + ipAdress);
        ...
        public AJsonRPC20 process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            if (cmdParams.customerId != null) {
                ...
                if (parallelCust == null) {
                    log.warn("PARALLEL:  User have no Customer for switching by customer ID=\"" + cmdParams.customerId + "\"" + " " + ipAdress);
                } else {
                    user.setCustomer(parallelCust);
                    log.debug("Юзер  \"" + user + "\" переключился  на кастомера \"" + parallelCust.getFullNumber() + "\"" + " " + ipAdress);
                }
            }
            log.warn("УДАЛЕНИЕ: Удалили по неявке кастомера " + user.getCustomer().getPrefix() + "-" + user.getCustomer().getNumber()
                    + " он ввел \"" + user.getCustomer().getInputData() + "\"" + " " + ipAdress);
            log.warn("REMOVING: Customer was removing because of absence " + user.getCustomer().getPrefix() + "-" + user.getCustomer().getNumber()
                    + " customer inputted \"" + user.getCustomer().getInputData() + "\"" + " " + ipAdress);
            ...
            if (user.getCustomer().getInputData() != null && !"".equals(user.getCustomer().getInputData())) {
                int cnt = 0;
                for (QService service : QServiceTree.getInstance().getNodes()) {
                    final LinkedList<QCustomer> forDel = new LinkedList<>();
                    service.getClients().stream().filter(customer -> (user.getCustomer().getInputData().equals(customer.getInputData()))).forEach(forDel::add);
                    forDel.stream().forEach(service::removeCustomer);
                    cnt = cnt + forDel.size();
                }
                if (cnt != 0) {
                    RpcBanList.getInstance().addToBanList(user.getCustomer().getInputData());
                }
                log.debug("Вместе с кастомером " + user.getCustomer().getPrefix() + "-" + user.getCustomer().getNumber()
                        + " он ввел \"" + user.getCustomer().getInputData() + "\" удалили еще его " + cnt + " проявлений." + " " + ipAdress);
            }
    ...
    final Task getStartCustomerTask = new Task(Uses.TASK_START_CUSTOMER) {
        ...
        public AJsonRPC20 process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            if (user.getCustomer() == null) {
                log.error("Не найден кастомер для начала работы с ним. А должен быть! Возможно из-за параллельного приема. user="
                        + user.getName() + "-" + user.getId() + " " + ipAdress);
    ...
    final Task customerToPostponeTask = new Task(Uses.TASK_CUSTOMER_TO_POSTPON) {
        ...
        public AJsonRPC20 process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            if (cmdParams.customerId != null) {
                ...
                if (parallelCust == null) {
                    ...
                    if (cmdParams.strict != null && !cmdParams.strict) {
                        CLIENT_TASK_LOCK.lock();
                        try {
                            log.debug("Warning! Random postponing was detected! \"" + cmdParams.customerId + "\"" + " " + ipAdress);
                            // бежим по очередям и ищем первого кастомера c ID
                            QCustomer customer = null;
                            for (QService service : QServiceTree.getInstance().getNodes()) {
                                if ((customer = service.gnawOutCustomerById(cmdParams.customerId)) != null) {
                                    log.debug("Warning!  Random postponing was successed! \"" + cmdParams.customerId + "\"" + " " + ipAdress);
                                    user.setCustomer(customer);
            ...
            try {
                ...
                log.error("Загнулось под конец." + " " + ipAdress, t);
            }
    ...
    final Task getFinishCustomerTask = new Task(Uses.TASK_FINISH_CUSTOMER) {
        ...
        public AJsonRPC20 process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            final QCustomer customer = user.getCustomer();
            if (customer == null) {
                log.error("Не найден кастомер для завершении работы с ним. А должен быть! Возможно из-за параллельного приема. user="
                        + user.getName() + "-" + user.getId() + " " + ipAdress);
                return new JsonRPC20Error(REQUIRED_CUSTOMER_NOT_FOUND, ipAdress + " " + user.toString() + "/" + user.getId());
            }
            // комменты
            ...
            if (backSrv != null) {
                log.debug("Требуется возврат после редиректа." + " " + ipAdress);
                ...
                log.info("Клиент \"" + user.getCustomer().getPrefix() + user.getCustomer().getNumber()
                        + "\" возвращен к услуге \"" + backSrv.getName() + "\"" + " " + ipAdress);
            } else {
                log.debug("В морг пациента." + " " + ipAdress);
                ...
                if (customer.getState() == CustomerState.STATE_FINISH && customer.getComplexId() != null) {
                    int len = 0;
                    len = customer.getComplexId().stream().map(List::size).reduce(len, Integer::sum);
                    if (len != 0) {
                        log.debug("Дефолтная проводка по комплексным услугам. Оcталось " + len + " " + ipAdress);
                        Long serviceID = null;
                ...
                if (customer.getState() == CustomerState.STATE_WAIT_COMPLEX_SERVICE) {
                    ...
                    //рассылаем широковещетельно по UDP на определенный порт
                    Uses.sendUDPBroadcast(customer.getService().getId().toString(), ServerProps.getInstance().getProps().getClientPort());
                    log.info("Клиент \"" + customer.getPrefix() + customer.getNumber() + "\" проведен по этапу к услуге \""
                            + customer.getService().getName() + "\"" + " " + ipAdress);
                }
            }
            try {
                user.setCustomer(null);//бобик сдох и медальки не осталось
    ...
    final Task redirectCustomerTask = new Task(Uses.TASK_REDIRECT_CUSTOMER) {
        ...
        public AJsonRPC20 process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            if (!oldService.getId().equals(cmdParams.serviceId)) {
                final QProperty redirectPriority = ServerProps.getInstance().getProperty(QProperties.SECTION_SYSTEM, "redirect_priority");
                // т.к. переставленный, то надо поменять ему приоритет.
                customer.setPriority(redirectPriority == null ? Uses.PRIORITY_HI : redirectPriority.getValueAsInt(Uses.PRIORITY_HI));
    ...
    final Task changeTempAvailableService = new Task(Uses.TASK_CHANGE_TEMP_AVAILABLE_SERVICE) {
        ...
        public synchronized AJsonRPC20 process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            log.trace("Изменена доступность для услуги \"" + service.getName() + "\" - "
    ...
    final Task saveBoardConfig = new Task(Uses.TASK_SAVE_BOARD_CONFIG) {
        ...
        public synchronized AJsonRPC20 process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            try {
                ...
                log.error("Не сохранилась конфигурация табло." + " " + ipAdress, ex);
            }
            return new JsonRPC20OK();
        }
    };
        ...
        public RpcGetGridOfDay process(final CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            if (!QCalendarList.getInstance().getById(1).checkFreeDay(startDay)
                ...
                if (!(interval.start == null || interval.finish == null)) {
                    ...
                    log.trace("Загрузим уже занятых позиций ранее записанными кастомерами от "
                            + dateFormat.format(startDay) + " до " + dateFormat.format(endDay) + " " + ipAdress);
                    // Загрузим уже занятых позиций ранее записанными кастомерами
                    final List<QAdvanceCustomer> advCustomers = Dao.get()
                            .find("FROM QAdvanceCustomer a WHERE advance_time >'" + dateFormat.format(startDay) + "' and advance_time <= '" + dateFormat.format(endDay)
    ...
    final Task getGridOfWeek = new Task(Uses.TASK_GET_GRID_OF_WEEK) {
        ...
        public RpcGetGridOfWeek process(final CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            final SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            log.trace("Загрузим уже занятых позиций ранее записанными кастомерами от "
                    + dateFormat.format(startWeek) + " до " + dateFormat.format(endWeek) + " " + ipAdress);
            // Загрузим уже занятых позиций ранее записанными кастомерами
            final List<QAdvanceCustomer> advCustomers = Dao.get()
                    .find("FROM QAdvanceCustomer a WHERE advance_time >'" + dateFormat.format(startWeek)
                            + "' and advance_time <= '" + dateFormat.format(endWeek) + "' and service_id = " + service.getId());
            ...
            while (day.before(endWeek)) {
                ...
                if (!QCalendarList.getInstance().getById(1).checkFreeDay(day)
                    ...
                    final QSchedule sch1 = service.getCalendar() == null
                            ? QCalendarList.getInstance().getById(1).getSpecSchedule(gcDay.getTime())
                            : (service.getCalendar().getSpecSchedule(gcDay.getTime()));
                    sch = (sch1 == null ? service.getSchedule() : sch1);
                    final QSchedule.Interval interval = sch.getWorkInterval(gcDay.getTime());
                        ...
                        while (start.before(end) || start.equals(end)) {
                            ...
                            if (!sch.inBreak(start, gc.getTime())) { // время не попало в перерыв
                                ...
                                for (QAdvanceCustomer advCustomer : advCustomers) {
                                    ...
                                    // Если совпал день и час и минуты, то увеличим счетчик записавшихся на этот час и минуты
                                    // тут учитываем что реально время предварительного может быть не по сетке, к примеру загрузили извне.
                                    if ((gc.get(GregorianCalendar.DAY_OF_YEAR) == gcDay.get(GregorianCalendar.DAY_OF_YEAR))
                                            && (s * 60 + s_m <= e * 60 + e_m
                                            && s * 60 + s_m + service.getAdvanceTimePeriod() > e * 60 + e_m)) {
                                        cnt++;
                                        // Защита от того чтобы один и тодже клиент не записался предварительно в одну услугу на одну дату.
    ...
    final Task standAdvanceInService = new Task(Uses.TASK_ADVANCE_STAND_IN) {
        ...
        @Override
        public synchronized RpcGetAdvanceCustomer process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            super.process(cmdParams, ipAdress, ip);

            final QService service = QServiceTree.getInstance().getById(cmdParams.serviceId);
            log.trace("Предварительно записываем c ID=" + cmdParams.customerId + " к услуге \"" + service.getName() + "\"(" + service.getPrefix() + "/" + cmdParams.serviceId + ")" + " ко времени " + new Date(cmdParams.date) + " " + ipAdress);
            // Создадим вновь испеченного кастомера
            ...
            if (cmdParams.customerId != -1) {
                Dao.get().load(acust, authCustonerID);
            ...
            log.debug("Старт сохранения предварительной записи в СУБД." + " " + ipAdress);
            Dao.get().execute(() -> {
                try {
                    Dao.get().saveOrUpdate(customer);
                    log.debug("Сохранили." + " " + ipAdress);
                } catch (Exception ex) {
    ...
    final Task standAdvanceCheckAndStand = new Task(Uses.TASK_ADVANCE_CHECK_AND_STAND) {
        ...
        public RpcStandInService process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            final QAdvanceCustomer advCust = Dao.get().find(QAdvanceCustomer.class, cmdParams.customerId);
            if (advCust == null || advCust.getId() == null || advCust.getAdvanceTime() == null) {
                log.debug("не найден клиент по его ID=" + cmdParams.customerId + " " + ipAdress);
                // Шлем отказ
                String answer = Locales.locMes("advclient_fail_number");
                return new RpcStandInService(answer);
            }
            final GregorianCalendar gc = new GregorianCalendar();
            ...
            if (new Date().before(gc1.getTime()) && new Date().after(gc.getTime())) {
                ...
                final Exception exception = Dao.get().execute(() -> {
                    Dao.get().delete(advCust);
                    log.debug("Удалили предварителньную запись о кастомере." + " " + ipAdress);
                });
                if (exception != null) {
                    throw new ServerException("Ошибка при удалении \n" + exception.toString() + "\n" + Arrays.toString(exception.getStackTrace()) + " " + ipAdress);
                }
                // создаем кастомера вызвав задание по созданию кастомера.
                // загрузим задание
                ...
                txtCustomer.getResult().getCustomer().setInputData(advCust.getInputData());
                return txtCustomer;
            } else {
    ...
    final Task removeAdvanceCustomer = new Task(Uses.TASK_REMOVE_ADVANCE_CUSTOMER) {
        ...
        public JsonRPC20OK process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            //трем запись в таблице предварительных записей
            Exception exception = Dao.get().execute(() -> {
                Dao.get().delete(advCust);
                log.debug("Удалили предварителньную запись о кастомере." + " " + ipAdress);
            });
            if (exception != null) {
        ...
        public AJsonRPC20 process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            if (cmdParams.userId != null && cmdParams.customerId == null) {
                if (QUserList.getInstance().hasById(cmdParams.userId)) {
                    ...
                    log.error("It is a bull shit! No user by id=\"" + cmdParams.userId + "\"" + " " + ipAdress);
                }
            }
            ...
            Dao.get().execute(() -> {
                try {
                    Dao.get().saveOrUpdate(event);
                    log.debug("Сохранили отзыв в базе." + " " + ipAdress);
                } catch (Exception ex) {
    ...
    final Task getClientAuthorization = new Task(Uses.TASK_GET_CLIENT_AUTHORIZATION) {
        ...
        public RpcGetAuthorizCustomer process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            final List<QAuthorizationCustomer> authCusts =
                    Dao.get().findByCriteria(DetachedCriteria.forClass(QAuthorizationCustomer.class)
            ...
            if (authCusts.isEmpty() || authCusts.get(0) == null || authCusts.get(0).getId() == null || authCusts.get(0).getName() == null) {
                log.trace("Не найден клиент по его ID = '" + cmdParams.clientAuthId + "'" + " " + ipAdress);
                authCust = null;
            } else {
    ...
    final Task setCustomerPriority = new Task(Uses.TASK_SET_CUSTOMER_PRIORITY) {
        ...
        public RpcGetSrt process(final CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            final String num = cmdParams.clientAuthId.replaceAll(REG_EXP_FORN, "");
            String hint = "";
            for (QService service : QServiceTree.getInstance().getNodes()) {
                if (service.changeCustomerPriorityByNumber(num, cmdParams.priority)) {
                    hint = "Клиенту с номером \"" + num + "\" в услуге \"" + service.getName() + "\" изменен приоритет.";
                    break;
                }
            }
            return new RpcGetSrt("".equals(hint) ? String.format(Locales.locMes("client_not_found_by_num"), num) : hint);
        }
    };
    /**
    ...
    final Task checkCustomerNumber = new Task(Uses.TASK_CHECK_CUSTOMER_NUMBER) {
        ...
        public RpcGetTicketHistory process(final CmdParams cmdParams, String ipAdress, byte[] ip) {
            super.process(cmdParams, ipAdress, ip);
            final String num = cmdParams.clientAuthId.trim().replaceAll(REG_EXP_FORN, "");
            String info = "";
            for (QService service : QServiceTree.getInstance().getNodes()) {
                for (QCustomer customer : service.getClients()) {
                    if (num.equalsIgnoreCase(customer.getPrefix() + (customer.getNumber() < 1 ? "" : customer.getNumber()))) {
                        info = String.format(Locales.locMes("client_with_number_to_service"), num, service.getName());
                        break;
                    }
                }
            }
            if ("".equals(info)) {
                for (QCustomer customer : QPostponedList.getInstance().getPostponedCustomers()) {
                    if (num.equalsIgnoreCase(customer.getPrefix() + (customer.getNumber() < 1 ? "" : customer.getNumber()))) {
                        info = String.format(Locales.locMes("client_with_number_postponed"), num);
                        break;
                    }
                }
            }
            ...
            if ("".equals(info)) {
                for (QUser user : QUserList.getInstance().getItems()) {
                    if (user.getCustomer() != null && num.equalsIgnoreCase(user.getCustomer().getFullNumber())) {
                        info = String.format(Locales.locMes("client_with_number_in_work"), num, user.getName());
                        break;
                    }
                }
            }

            if ("".equals(info) && KILLED_CUSTOMERS.get(num) != null) {
                info = String.format(Locales.locMes("client_with_number_removed"), num, Locales.getInstance().formatForLabel.format(KILLED_CUSTOMERS.get(num)));
            }

            final String n = num.replaceAll("\\D+", "");
            final String p = num.replaceAll(n, "");
            final List<QCustomer> custs = Dao.get().find("FROM QCustomer a WHERE service_prefix ='" + p + "' and number = " + (n.isEmpty() ? "0" : n));
            final LinkedList lc = new LinkedList();
            final SimpleDateFormat dateFormat = new SimpleDateFormat("dd.MM.yyyy HH:mm:ss");
            custs.forEach(cust ->
                    lc.add(dateFormat.format(cust.getStandTime()) + " &nbsp;&nbsp;&nbsp;&nbsp;"
                            + (cust.getService().getName().length() > 96
                            ? cust.getService().getName().substring(0, 95) + "..."
                            : cust.getService().getName())
                            + "&nbsp;&nbsp;&nbsp;&nbsp;" + cust.getUser().getName() + "&nbsp;&nbsp;&nbsp;&nbsp;" + CustomerState.values()[cust.getStateIn()])
            );
            return new RpcGetTicketHistory(new RpcGetTicketHistory.TicketHistory("".equals(info) ? String.format(Locales.locMes("client_not_found_by_num_at_all"), num) : info,
                    lc));
        }
    };
    /**
    ...
    final Task refreshRunningText = new Task(Uses.TASK_CHANGE_RUNNING_TEXT_ON_BOARD) {
        ...
        public AJsonRPC20 process(final CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            if (MainBoard.getInstance() instanceof QIndicatorBoardMonitor) {
                ...
                if (Uses.TAG_BOARD_TOP.equalsIgnoreCase(cmdParams.infoItemName)) {
                    updateSegment(mon.indicatorBoard.getPanelUp(), mon.indicatorBoard.getTopRunningLabel());
                }
                if (Uses.TAG_BOARD_LEFT.equalsIgnoreCase(cmdParams.infoItemName)) {
                    updateSegment(mon.indicatorBoard.getPanelLeft(), mon.indicatorBoard.getLeftRunningLabel());
                }
                if (Uses.TAG_BOARD_RIGHT.equalsIgnoreCase(cmdParams.infoItemName)) {
                    updateSegment(mon.indicatorBoard.getPanelRight(), mon.indicatorBoard.getRightRunningLabel());
                }
                if (Uses.TAG_BOARD_BOTTOM.equalsIgnoreCase(cmdParams.infoItemName)) {
                    updateSegment(mon.indicatorBoard.getPanelDown(), mon.indicatorBoard.getBottomRunningLabel());
                }
                if (Uses.TAG_BOARD_BOTTOM_2.equalsIgnoreCase(cmdParams.infoItemName)) {
                    updateSegment(mon.indicatorBoard.getPanelDown2(), mon.indicatorBoard.getBottom2RunningLabel());
                }
            }
            return new JsonRPC20OK();
        }

        private void updateSegment(QPanel panel, RunningLabel label) {
            panel.closeVideo();
            panel.removeAll();
            ...
            label.setRunningText(cmdParams.textData);
            label.start();
        }
    };
    /**
    ...
    final Task saveProperties = new Task(Uses.TASK_SAVE_PROPERTIES) {
        ...
        public RpcGetProperties process(CmdParams cmdParams, String ipAdress, byte[] ip) {
            super.process(cmdParams, ipAdress, ip);
            cmdParams.properties.forEach(prop -> ServerProps.getInstance().saveOrUpdateProperty(prop));
    ...
    final Task reinitRoll = new Task(Uses.TASK_REINIT_ROLL) {
        ...
        public AJsonRPC20 process(final CmdParams cmdParams, String ipAdress, byte[] ip) {
            ...
            final QProperty prop = ServerProps.getInstance().getProperty(service.getSectionName(), Uses.KEY_TICKET_NUMBERING,
                    "from " + cmdParams.first + " to " + cmdParams.last, "must contain two numbers");
    ...
    public Object doTask(JsonRPC20 rpc, String ipAdress, byte[] ip) {
        ...
        if (!QConfig.cfg().isDebug()) {
            ln("Task processing: '" + rpc.getMethod() + " " + ipAdress);
        }
        log.info("Task processing: '" + rpc.getMethod() + "'" + " " + ipAdress);
        ...
        //ВЫЗОВ КОНКРЕТНОЙ КОМАНДЫ. ВЫЗОВ ОБЕРНУТ В МЕТОДЫ ПЛАГИНОВ. ПЛАГИНЫ МОГУТ ПЕРЕКРЫТЬ ВЫЗОВ САМОЙ КОМАНДЫ.
        // поддержка расширяемости плагинами
        // Вызов обертки из плагина, если плагин что-то вернул, то считаем команду выполненной.
        AJsonRPC20 jsonRPC20 = null;
        for (final IServerTaskWrapper handler : welcomeServerHandlers) {
            log.info("Вызов SPI расширения Before. Описание: " + handler.getDescription());
            try {
                final AJsonRPC20 mediate = handler.handleStandInParamsOnServerBefore(tasks.get(rpc.getMethod()), rpc.getParams(), ipAdress, ip);
                if (mediate != null) {
                    jsonRPC20 = mediate;
                }
            } catch (Exception tr) {
        ...
        if (jsonRPC20 == null) {
            ...
            result = tasks.get(rpc.getMethod()).process(rpc.getParams(), ipAdress, ip); //ВЫЗОВ КОНКРЕТНОЙ КОМАНДЫ.
            for (final IServerTaskWrapper handler : welcomeServerHandlers) {
                log.info("Вызов SPI расширения After. Описание: " + handler.getDescription());
                try {
                    final AJsonRPC20 mediate = handler.handleStandInParamsOnServerAfter(tasks.get(rpc.getMethod()), rpc.getParams(), ipAdress, ip, result);
                    if (mediate != null) {
                        jsonRPC20 = mediate;
                    }
                } catch (Exception tr) {
        ...
        log.info("Task was finished. Time: " + ((double) (System.currentTimeMillis() - start)) / 1000 + " sec." + " " + ipAdress);
        return result;
    }

    /**
     * Загрузим плагины для применения их в обработчиках заданий. Чтоб постоянно их не грузить при каждом задании.
     */
    final ServiceLoader<IServerTaskWrapper> welcomeServerHandlers = ServiceLoader.load(IServerTaskWrapper.class);
}
