...
/**
 * синглтон со списком отложенных.
 *
...
public class QPostponedList extends DefaultListModel {
     ...
     * Загрузим отложенных.
     *
     * @param customers из них формируем список для гуйни и прочего.
     * @return синглтон со списком.
     */
    public QPostponedList loadPostponedList(List<QCustomer> customers) {
        clear();
                    ...
                    try {
                        ...
                        for (QCustomer customer : getPostponedCustomers()) {
                            if (customer.getPostponPeriod() > 0 && customer.getFinishPontpone() < System.currentTimeMillis()) {
                                log.debug("Перемещение по таймеру из отложенных кастомера №" + customer.getPrefix() + customer.getNumber() + " в услугу \"" + customer.getService().getName() + "\"");
                        ...
                        forDel.stream().forEach(this::removeElement);
                    } catch (Exception ex) {
    ...
    public QCustomer getById(long id) {
        for (Object object : toArray()) {
            QCustomer customer = (QCustomer) object;
