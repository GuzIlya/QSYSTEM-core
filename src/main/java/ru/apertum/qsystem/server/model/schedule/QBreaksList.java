...
public class QBreaksList extends ATListModel<QBreaks> implements ComboBoxModel {
    ...
    @Override
    public void save() {
        //сохраняем тут, т.к. из-за констрейнта перерывов нужно учесть "ON DELETE CASCADE ON UPDATE CASCADE" в БД и @Cascade({CascadeType.SAVE_UPDATE})
        Dao.get().saveOrUpdateAll(getItems());
        getItems().stream().forEach(breaks -> Dao.get().saveOrUpdateAll(breaks.getBreaks()));
    }

    /**
     * Удаленные QBreaks надо подтереть этим методом. Они накапливаются в списке и требуют ручной персистенции.
     */
    public void deleteRemoved() {
        // трем QBreaks, breaks в составе удалятся самой БД по констрейнту.
        Dao.get().deleteAll(deleted);
        deleted.clear();
    }

    @Override
    public boolean removeElement(QBreaks breaks) {
        // удаляем только сами QBreaks, breaks в составе удалятся самой БД по констрейнту.
        breaks.getBreaks().clear();
        return super.removeElement(breaks);
    }
}
