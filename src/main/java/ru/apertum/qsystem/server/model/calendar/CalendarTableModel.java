...
public class CalendarTableModel extends AbstractTableModel {
    ...
    public CalendarTableModel(QCalendar calc) {
        log.debug("Создаем модель для календаря");
        this.calendar = calc;
    ...
    public void checkSunday(int year) {
        log.debug("Пометим все воскресенья");
        final GregorianCalendar gc = new GregorianCalendar();
        gc.set(GregorianCalendar.YEAR, year);
        final int ye = year % 4 == 0 ? 366 : 365;
        for (int i = 1; i <= ye; i++) {
     ...
     */
    public void save() {
        log.info("Сохраняем календарь \"" + calendar + "\" ID=" + calendar.getId());
        ...
        transaction.commit();
        log.debug("Сохранили новый календарь");
