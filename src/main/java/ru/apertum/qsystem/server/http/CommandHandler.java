...
/**
 * Обработчик команд при обращении по HTTP.
 *
    ...
    public void handle(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response) {
        ...
        switch (target) {
            ...
            case INFO_URL_PATTERN:
                log.trace("HTTP task: " + INFO_URL_PATTERN);
                ...
                contentType = "text/html;charset=" + StandardCharsets.UTF_8;
                break;
            case CMD_URL_PATTERN:
                final JsonRPC20 rpc;
                if ("GET".equalsIgnoreCase(request.getMethod())) {
                    log.trace("HTTP GET task: \"" + request.getParameter(CmdParams.CMD) + "\"\n" + request.getQueryString());
                    ...
                    try {
                        ...
                        try (ServletInputStream fis = request.getInputStream(); Scanner s = new Scanner(new InputStreamReader(fis, StandardCharsets.UTF_8))) {
                            while (s.hasNextLine()) {
                        ...
                        log.error("Error of reading input data on http.", ex);
                        break;
                    }
                    if (data == null || data.isEmpty()) {
                        ...
                        contentType = "text/html;charset=utf-8";
                        log.error("Не получен текст команды по http..");
                        break;
                    }
                    log.trace("HTTP POST task:\n" + (data.length() > 200 ? (data.substring(0, 200) + "...") : data));
                ...
                try {
                    ...
                    f = true;
                    log.error("Произошла ошибка обработки задания. ", ex);
                } finally {
                    GsonPool.getInstance().returnGson(gson);
                }
                result = answer;
