...
public class NetCommanderTest {
    ...
    @Test(groups = "NetCommander", dependsOnMethods = "testTotalCircleForAddedFireService")
    public void testTotalCircleForChangePriprity() throws Exception {
        boolean check = NetCommander.getSelfServicesCheck(netProperty, user.getId());
        assertTrue(check);
        ...
        // Второму поднимим приоритет
        String serviseFire = NetCommander.setCustomerPriority(netProperty, 2, "bad number");
        assertEquals(serviseFire, "A customer was not foung by number 'badnumber'.");
        serviseFire = NetCommander.setCustomerPriority(netProperty, 2, customer2.getFullNumber());
        assertNotNull(serviseFire);
        assertTrue(serviseFire.contains(customer2.getFullNumber()));

        // второй стоит
        RpcGetTicketHistory.TicketHistory ticketHistory = NetCommander.checkCustomerNumber(netProperty, customer2.getFullNumber());
        assertNotNull(ticketHistory);
        assertTrue(ticketHistory.getInfo().contains(customer2.getFullNumber()));
        assertTrue(ticketHistory.getInfo().contains(service.getName()));

        // вызовем первого, того что с повышенным приоритетом, он теперь первее
        QCustomer invitedCustomer = NetCommander.inviteNextCustomer(netProperty, user.getId());
        assertEquals(customer2, invitedCustomer);
        // обработаем
        NetCommander.getStartCustomer(netProperty, user.getId());
        ...
        // приоритетного больше нет
        ticketHistory = NetCommander.checkCustomerNumber(netProperty, customer2.getFullNumber());
        assertNotNull(ticketHistory);
        assertTrue(ticketHistory.getInfo().contains(customer2.getFullNumber()));
        assertTrue(ticketHistory.getInfo().contains("is not found"));
    ...
    @Test(groups = "NetCommander", dependsOnMethods = "testTotalCircleForChangePriprity")
    public void testTotalCircleForAdvanced() throws Exception {
        boolean check = NetCommander.getSelfServicesCheck(netProperty, user.getId());
        assertTrue(check);
        ...
        // ставим двух предварительно
        QAdvanceCustomer advCustomer1 = NetCommander.standInServiceAdvance(netProperty, service.getId(), new Date(), -1, "input1", "comment1");
        assertNotNull(advCustomer1);
        assertEquals(advCustomer1.getInputData(), "input1");
        assertTrue(advCustomer1.getId() - System.currentTimeMillis() / 100000 < 200);
        QAdvanceCustomer advCustomer2 = NetCommander.standInServiceAdvance(netProperty, service.getId(), new Date(), -1, "input2", "comment2");
        assertNotNull(advCustomer2);
        assertEquals(advCustomer2.getInputData(), "input2");
        assertTrue(advCustomer2.getId() - System.currentTimeMillis() / 100000 < 200);

        // первый пришел и получил талон в очередь, он в услуге и может быть вызван оператором.
        RpcStandInService.AdvResult advResult = NetCommander.standAndCheckAdvance(netProperty, advCustomer1.getId());
        assertNotNull(advResult);
        assertNull(advResult.getError());
        assertEquals(advResult.getCustomer().getInputData(), "input1");

        // вызовем первого
        QCustomer invitedCustomer = NetCommander.inviteNextCustomer(netProperty, user.getId());
        assertEquals(advResult.getCustomer(), invitedCustomer);
        // обработаем
        NetCommander.getStartCustomer(netProperty, user.getId());
        QCustomer finishCustomer = NetCommander.getFinishCustomer(netProperty, user.getId(), null, -1L, "");
        assertEquals(advResult.getCustomer(), finishCustomer);

        // Вызываем по неправильному номеру
        advResult = NetCommander.standAndCheckAdvance(netProperty, 555L);
        assertNotNull(advResult);
        assertNotNull(advResult.getError());
        assertNull(advResult.getCustomer());

        // удаляем предварительно по неправильному номеру
        JsonRPC20OK ok = NetCommander.removeAdvancedCustomer(netProperty, 555L);
        assertNotNull(ok);
        assertEquals(ok.getResult(), ADVANCED_NOT_FOUND.getCode().intValue()); // ADVANCED_NOT_FOUND(4, "Не верный номер предварительной записи."),

        // удаляем предварительного по правильному номеру, должен удалиться
        ok = NetCommander.removeAdvancedCustomer(netProperty, advCustomer2.getId());
        assertNotNull(ok);
        assertEquals(ok.getResult(), 1);

        // второй пытается получить талон, но он уже удален админом.
        advResult = NetCommander.standAndCheckAdvance(netProperty, advCustomer2.getId());
        assertNotNull(advResult);
        assertNull(advResult.getCustomer());
        assertNotNull(advResult.getError());
    }
